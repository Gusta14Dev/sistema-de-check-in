<x-app-layout>
    ...
    <x-slot name="checkin">true</x-slot>
                    ...
                    <div class="w-full flex justify-between shadow-md flex-wrap mb-5">
                        <div class="bg-gradient-to-r from-blue-500 to-blue-700 w-full sm:w-2/5 flex justify-around items-center py-2 rounded-t-md sm:rounded-tr-none sm:rounded-l-md">
                            <div class="flex flex-col justify-around  items-center text-white">
                                <p class="font-bold">{{$presence->room->name}}</p>
                                <p class="text-xs lg:text-base">Professor {{$presence->room->teacher}}</p>
                            </div>
                        ...
                        <div class="w-full sm:w-3/5 h-24 sm:h-auto rounded-b-md sm:rounded-r-md px-2 sm:px-8 py-4 flex flex-col justify-between text-xs sm:text-base shadow-inner">
                            <div class="flex justify-between items-center">
                                <p class="text-blue-600 text-sm font-bold">Começo: {{$presence->room->start}}</p>

                                @if(Auth::user()->isAdmin)
                                <div class="flex justify-around w-32">
                                    <button class="border border-blue-600 hover:bg-blue-600 hover:text-white text-blue-600 w-6 h-6 rounded flex justify-center items-center mr-2 transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 21h7a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v11m0 5l4.879-4.879m0 0a3 3 0 104.243-4.242 3 3 0 00-4.243 4.242z" />
                                        </svg>
                                    </button>
                                    <button class="border border-yellow-500 hover:bg-yellow-500 hover:text-white text-yellow-500 w-6 h-6 rounded flex justify-center items-center mr-2 transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                            <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                                            <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" />
                                        </svg>
                                    </button>
                                    <button class="border border-red-600 hover:bg-red-600 hover:text-white text-red-600 w-6 h-6 rounded flex justify-center items-center mr-2 transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                @else
                                    @if($presence->room->getPresenceUserOfRoom(Auth::user()->id, $presence->room->id))
                                        @if(($presence->room->start->diffInMinutes($now)) > 30)
                                            <div class="flex justify-around w-36">
                                                <form action="{{route('checkIn', $presence->room->id)}}" method="post">
                                                    @csrf
                                    ...
                                    @else
                                        @if(($presence->room->start->diffInMinutes($now)) > 30)
                                            <div class="flex justify-around w-36">
                                                <form action="{{route('checkIn.cancel', $presence->room->id)}}" method="post">
                                                    @csrf
                            ...
                            <div class="flex justify-between">
                                <p class="text-blue-600 text-sm font-bold">Quantidade de Corfimados: {{count($presence->room->presences)}}</p>
                            </div>
                            <div class="flex justify-between">
                                <p class="text-blue-600 text-sm font-bold">Maximo de alunos: {{$presence->room->max_students}}</p>
                            </div>
                            @else
                            <div class="flex justify-between">
                                <p class="text-blue-600 text-sm font-bold">Termino: {{$presence->room->final}}</p>
                            </div>
                            <div class="flex justify-between">
                                <p class="text-blue-600 text-sm font-bold">Vagas: {{($presence->room->max_students - count($presence->room->presences))}}</p>
                            </div>
                            @endif
                        </div>
                    </div>
